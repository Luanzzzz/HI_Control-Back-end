...
class NotaFiscalBase(BaseModel):
    ...
    """
    numero_nf: str = Field(..., min_length=1, max_length=20, description="Numero da nota fiscal")
    serie: str = Field(..., min_length=1, max_length=10, description="Serie da nota")
    ...
    chave_acesso: Optional[str] = Field(None, min_length=44, max_length=44, description="Chave de acesso de 44 digitos")

    data_emissao: datetime = Field(..., description="Data e hora de emissao")
    data_autorizacao: Optional[datetime] = Field(None, description="Data e hora de autorizacao")
    ...
    situacao: str = Field(default="processando", description="Situacao atual da nota")
    ...
    def validar_chave_acesso(cls, v: Optional[str]) -> Optional[str]:
        ...
        if not v.isdigit() or len(v) != 44:
            raise ValueError("Chave de acesso deve conter exatamente 44 digitos numericos")
        return v
...
class NotaFiscalDetalhada(NotaFiscalResponse):
    """Schema com informacoes completas incluindo impostos"""
